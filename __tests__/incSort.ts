...
describe('sort', () => {
  it('sorts a sequence', () => {
    expect(Seq([4, 5, 6, 3, 2, 1]).incSort().toArray()).toEqual([
      1, 2, 3, 4, 5, 6,
    ]);
  });

  it('sorts a list', () => {
    expect(List([4, 5, 6, 3, 2, 1]).incSort().toArray()).toEqual([
      1, 2, 3, 4, 5, 6,
    ]);
  });
  ...
  it('sorts undefined values last', () => {
    expect(
      List([4, undefined, 5, 6, 3, undefined, 2, 1]).incSort().toArray()
    ).toEqual([1, 2, 3, 4, 5, 6, undefined, undefined]);
  });

  it('sorts a keyed sequence', () => {
    expect(
      Seq({ z: 1, y: 2, x: 3, c: 3, b: 2, a: 1 }).incSort().entrySeq().toArray()
    ).toEqual([
      ['z', 1],
      ['a', 1],
      ['y', 2],
